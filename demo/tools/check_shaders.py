...
Slope's check (by way of clamp's), in this repo's shape.

------------------------------------------------------------------- why

`demo/plugin.js` holds three GLSL strings and so does `source/Shaders.cpp`.
...
something enforcing it. For this plugin the shaders are the chain: every stage
lives once, in GLSL, and the C++ only computes the constants they read.

Nothing else can. `cctest` drives the real plugin class through a real FFGL
...
constants the plugin computes in double each frame (`Model.h`: the OETF's a and
k, the matrices, the drift's step) and the two draws with their uniforms
(`Ccu::ProcessOpenGL`) are a hand translation in plugin.js, and only a reader
can tell whether they still agree. When you change one of those, change it here
too -- and remember that a wrong constant shows up on the page as a picture that
...
SHADERS = [
    ...
    ("LINEAR", "source/Shaders.cpp", "kLinear"),
    ("PROCESS", "source/Shaders.cpp", "kProcess"),
]
