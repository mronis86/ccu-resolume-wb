...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/cctest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to have everything the chain acts on.** A grey ramp (gamma,
knee, black gamma, pedestal, clip), saturated patches with a skin tone and a
warm white (matrix, saturation, white balance, the knee's per-channel clip),
a face with fine hair against a textured jacket (skin detail against the
rest), bars of falling width and low-contrast bars (detail, crispening
frequency, coring), a warm sky (knee), and a dark textured floor (coring,
level dependence). `cctest --out` renders it.

**Some controls need another to mean anything.** Skin Hue and Skin Width
only exist while Skin Detail is up; Knee Point and Knee Slope only while the
knee is on; Coring and Level Dependence only while there is detail to core.
And Skin Hue's two ends are the same hue (0 and 360 degrees), so it is
compared at 0 and 180. CONTEXT says what else has to be true for each, and
uses nothing but the plugin's own parameters.

**Drift is slow by design.** Its time constant is 20 s, so at 40 frames the
walk has moved a quarter of its final excursion; that is enough to show at
Drift 1, and the sweep says so with a frame count of its own.

**An option's range is its element count.** An option parameter's range
reads back 0..1 whatever its element count, so `cctest --list` prints the
count as the range, and the ends swept are the first and last element.
...
CONTEXT = {
    "Skin Hue": {"Skin Detail": 1, "_high": 0.5},
    "Skin Width": {"Skin Detail": 1},
    "Knee Point": {"Knee On": 1},
    "Knee Slope": {"Knee On": 1},
    "Coring": {"Detail Level": 0.5},
    "Level Dependence": {"Detail Level": 0.5},
    "Drift": {"_frames": 60},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "cctest"))
    ...
    scratch = tempfile.mkdtemp(prefix="ccsweep")
